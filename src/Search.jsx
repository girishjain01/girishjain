...
function Search1() {
  ...
  const filteredData = data.filter(item => {
    const searchContent = transliterate(searchTerm);
    return (
      item.name?.toLowerCase().includes(searchContent) ||
      item.heading?.toLowerCase().includes(searchContent) ||
      item.arth?.toLowerCase().includes(searchContent) ||
      item.sutar?.toLowerCase().includes(searchContent) ||
      item.description?.toLowerCase().includes(searchContent)
    );
  });

  return (
    <div className="p-10 bg-light pt-28">
